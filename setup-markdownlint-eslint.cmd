...
rem Capturar o número da compilação do Windows usando PowerShell (substitui o uso de wmic, que foi descontinuado)
for /f "usebackq tokens=*" %%i in (`powershell -NoProfile -Command "(Get-ItemProperty 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion').CurrentBuildNumber"`) do (
    set "BUILD=%%i"
)

rem Verificação extra: garantir que a variável BUILD foi definida corretamente
if not defined BUILD (
    echo %vermelho%ERRO: Não foi possível capturar o número da build do Windows.%reset%
    echo Certifique-se de que você tem permissões de administrador e está executando em um Windows compatível.
    pause
    exit /b 1
)
...
if !BUILD! geq 26100 (
    echo Sistema Operacional: Windows 11 24H2 ou superior.
) else if !BUILD! geq 22631 (
    echo Sistema Operacional: Windows 11 23H2.
